...
BEGIN
    ...
    EXECUTE 
        CASE
            WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'user_preferences' AND column_name = 'aiPreferito' AND column_default IS NULL) THEN
                'ALTER TABLE public.user_preferences ALTER COLUMN "aiPreferito" SET DEFAULT \'{}\''::jsonb;'
            ELSE ''
        END;
    EXECUTE 
        CASE
            WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'user_preferences' AND column_name = 'budget' AND column_default IS NULL) THEN
                'ALTER TABLE public.user_preferences ALTER COLUMN budget SET DEFAULT \'{"mensile":50,"disponibileNuoviTool":true}\''::jsonb;'
            ELSE ''
        END;
    EXECUTE 
        CASE
            WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'user_preferences' AND column_name = 'notifiche' AND column_default IS NULL) THEN
                'ALTER TABLE public.user_preferences ALTER COLUMN notifiche SET DEFAULT \'{"suggerimenti":true,"nuoveFunzionalita":true}\''::jsonb;'
            ELSE ''
        END;
    EXECUTE 
        CASE
            WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_schema = 'public' AND table_name = 'user_preferences' AND column_name = 'tema' AND column_default IS NULL) THEN
                'ALTER TABLE public.user_preferences ALTER COLUMN tema SET DEFAULT \'light\';'
            ELSE ''
        END;
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_user_preferences_aipreferito_object') THEN
        ALTER TABLE public.user_preferences ADD CONSTRAINT chk_user_preferences_aipreferito_object CHECK (jsonb_typeof("aiPreferito") = 'object');
    END IF;
