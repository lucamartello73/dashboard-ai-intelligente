...
import type { SpazioAI, Computer } from "@/types/database"
...
export class OrchestratoreAI {
  ...
  async analizzaRichiesta(richiesta: string) {
    try {
      ...
      const [spaziAI, computer] = await Promise.all([this.getSpazi(), this.getComputer()])
...
`
  ...
  private async getSpazi(): Promise<SpazioAI[]> {
    const { data, error } = await supabaseAdmin.from("Spazi_AI").select("*")
  ...
  private async getComputer(): Promise<Computer[]> {
    const { data, error } = await supabaseAdmin.from("Computer").select("*")
